...
const Navbar : React.FC<{showSidebar: boolean; setShowSidebar: React.Dispatch<React.SetStateAction<boolean>> ,data: Array<string>, pageName: string, state: [string, React.Dispatch<React.SetStateAction<string>>] }> = ({state, showSidebar, setShowSidebar }) => {
    ...
    const toggleSidebar = () => {
     ...
     return (
      <nav className="fixed backdrop-blur bg-gray-50 transition-colors dark:border-slate-50/[0.06] bg-white/10 supports-backdrop-blur:bg-white/60 dark:bg-gray-700/[0.8] top-0 z-30 w-full p-4 h-20 flex justify-between">
